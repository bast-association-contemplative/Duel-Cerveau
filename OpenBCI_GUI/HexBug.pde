...
class HexBug {
  ...
  HexBug(Serial serialPort) {
    int ID = 0;
    command_climb = new Command(serialPort,"r","Climb",ID++);
    command_dive = new Command(serialPort,"b","Dive",ID++);
    command_left = new Command(serialPort,"r","Left",ID++);
    command_right = new Command(serialPort,"b","Right",ID++);
    command_forward = new Command(serialPort,"r","Swim!",ID++);
  }
